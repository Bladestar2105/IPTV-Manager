...
## 2024-05-24 - SSRF Bypass via Client-Signed Payload
**Vulnerability:** The `/live/segment` endpoint accepted a Base64-encoded `data` parameter from the client, which contained the upstream URL. The application trusted this parameter to bypass SSRF checks, assuming it was generated by the server. This allowed attackers to craft malicious payloads pointing to internal services.
**Learning:** Never trust client-side data for security decisions (like skipping validation), even if it "looks" like internal data. Base64 encoding provides no integrity.
**Prevention:** Use symmetric encryption (AES) or HMAC signatures for sensitive data round-tripped through the client. Ensure critical security checks (like SSRF validation) run on *all* requests, regardless of source.

## 2026-02-13 - Insecure Encryption Mode Upgrade
**Vulnerability:** The `encrypt` utility used `aes-256-cbc` without an HMAC, making it vulnerable to Bit-Flipping and Padding Oracle attacks.
**Learning:** Legacy encryption functions often lack integrity checks. Upgrading them requires a strategy for backward compatibility (e.g., detecting format by number of parts).
**Prevention:** Always use Authenticated Encryption (like AES-GCM) for new implementations. When auditing, check `crypto.createCipheriv` algorithms.
