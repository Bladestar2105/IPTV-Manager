...
| `AI_CODEX_BIN` | `codex` | Path to the pinned Codex CLI, or a bare name resolved once against `PATH`. The resolved absolute path is both version-probed and launched, and its directory is bound read-only inside the sandbox. Its reported version must fall inside the tested range (see [AI setup](AI_INTEGRATION.md)). |
