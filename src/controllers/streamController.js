...
function getStat(channelId) {
  if (!stmts.getStat)
    stmts.getStat = db.prepare(
      "SELECT id FROM stream_stats WHERE channel_id = ?",
    );
...
function updateStat(lastViewed, id) {
  if (!stmts.updateStat)
    stmts.updateStat = db.prepare(
      "UPDATE stream_stats SET views = views + 1, last_viewed = ? WHERE id = ?",
    );
...
function insertStat(channelId, lastViewed) {
  if (!stmts.insertStat)
    stmts.insertStat = db.prepare(
      "INSERT INTO stream_stats (channel_id, views, last_viewed) VALUES (?, 1, ?)",
    );
...
function getProvider(id) {
  if (!stmts.getProvider)
    stmts.getProvider = db.prepare("SELECT * FROM providers WHERE id = ?");
...
function isBrowser(req) {
  ...
  return /Chrome|Firefox|Safari|Edge|OPR|Opera|Vivaldi|Brave|SamsungBrowser|UCBrowser|MSIE|Trident/i.test(
    ua,
  );
}
...
async function fetchWithBackups(primaryUrl, backupUrls, options) {
  ...
  const fetchOptions = {
    ...options,
    agent: (_parsedUrl) =>
      _parsedUrl.protocol === "https:" ? httpsAgent : httpAgent,
  };
  ...
  try {
    ...
    if (meta && meta.original_url) {
      if (relativePath === "manifest.mpd" || relativePath === "") {
        ...
        // Prevent open proxy / SSRF via absolute or protocol-relative URLs in relativePath
        const trimmedPath = relativePath.trim();
        try {
          // If it can be parsed as a URL without a base, it's absolute
          new URL(trimmedPath);
          return res.status(400).send("Invalid relative path");
        } catch (e) {
          // Not an absolute URL, but check for protocol-relative //
    ...
    if (user.is_share_guest) {
      ...
      if (
        (user.share_start && nowSec < user.share_start) ||
        (user.share_end && nowSec > user.share_end)
      )
        return res.sendStatus(403);
    }

    if (relativePath.endsWith(".mpd")) {
      await streamManager.add(
        connectionId,
        user,
        `${channel.name} (DASH)`,
        req.ip,
...
export const proxyLive = async (req, res) => {
  ...
  try {
    ...
    if (shouldTranscode) {
      console.log(
        `🎬 Starting audio transcoding for stream ${streamId} (${reqExt})`,
      );
    ...
    try {
      ...
      console.error(
        `Stream proxy error: ${e.message} for ${redactUrl(remoteUrl)}`,
      );
      streamManager.remove(connectionId);
      return res.sendStatus(502);
    }
...
export const proxySegment = async (req, res) => {
  try {
    ...
    if (!upstream.ok) {
      console.error(
        `⚠️ Segment upstream error: ${upstream.status} for ${targetUrl}`,
      );
...
export const proxyMovie = async (req, res) => {
  ...
  try {
    ...
    const shouldTranscode =
      ...
      (isBrowser(req) && (ext === "mkv" || ext === "avi"));

    if (shouldTranscode) {
      console.log(
        `🎬 Starting VOD transcoding for stream ${streamId} (${ext} -> mp4)`,
      );
...
export const proxySeries = async (req, res) => {
  ...
  try {
    ...
    await streamManager.add(
      connectionId,
      user,
      `Series Episode ${remoteEpisodeId}`,
      req.ip,
      res,
    );
