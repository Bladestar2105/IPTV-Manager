...
function getEpgInfo(session, params) {
  ...
  const epgIds = new Set(channels.map(channel => channel.epg_id).filter(Boolean));
  const programsByEpgId = getEpgProgramsForChannels(epgIds, start, end);

  for (const channel of channels) {
    if (!channel.epg_id) continue;
    data[String(channel.id)] = (programsByEpgId.get(channel.epg_id) || [])
...
function createArchiveLink(req, session, command) {
  const match = command.match(/\/media\/stalker_archive_(\d+)_(\d+)_(\d+)\.mpg/i);
...
function createLink(req, session, params, requestedType) {
  ...
  const episodeNumber = Number(value(params, 'series'));
  if (target.type === 'series' || (requestedType === 'vod' && Number.isSafeInteger(episodeNumber) && episodeNumber > 0)) {
    return createSeriesLink(req, session, target, params);
  }

  const type = requestedType === 'vod' ? 'vod' : target.type;
  if (type !== target.type && !(type === 'itv' && target.type === 'radio')) return emptyLink();
